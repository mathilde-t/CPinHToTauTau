...
def create_cutflow_histogram(cuts, data, xlabel="Selections", ylabel="Selection efficiency", title="", log=False, rel=False, save_path=None):
    ...
    print(f"n_evt: {n_evt}")
    print(f"n_evt[0]: {n_evt[0]}")
    print(f"Logarithm of n_evt[0]: {np.log10(n_evt[0]) if n_evt[0] > 0 else 'Undefined'}")
...
def get_hist_values(pickle_file):
    ...
    values = []
    print(hist)
    steps = data.axes['step'] # or hist.axes['step']
...
create_cutflow_histogram(cuts, 
                         ...
                         save_path="cutflow_histogram_npvs.png",
                         ylabel="N evt",
                         log=True,
                         rel=False)